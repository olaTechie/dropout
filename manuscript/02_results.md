...
The top three features by mean |SHAP| value for T1 were **h69** (recorded vaccination status), **Travel_Times** (geospatial travel time to the nearest health facility), and **Nightlights_Composite** (a satellite-derived proxy for local development and infrastructure), consistent with the dominance of dynamic timing and community-enabling factors at the first dropout step.
...
The top three features by mean |SHAP| value for T2 were **com_zses** (community zonal socioeconomic composite), **h22** (recent febrile illness in the child, a proxy for recent care-seeking contact), and **v137** (number of children under five in the household), reflecting a shift at T2 toward community-level socioeconomic structure and household composition rather than individual cascade timing.
