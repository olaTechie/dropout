...
A 2 × 2 panel figure. Panel A (top-left): receiver-operating-characteristic (ROC) and precision-recall (PR) curves for the T1 (DTP1→DTP2) XGBoost model after isotonic recalibration (AUROC 0.910, 95% CI 0.885–0.932; AUPRC 0.299). Panel B (top-right): ROC and PR curves for the T2 (DTP2→DTP3) model (AUROC 0.943, 95% CI 0.929–0.957; AUPRC 0.548). Panel C (bottom-left): SHAP mean-absolute-value bar chart for T1 — top-3 predictors are h69 (recorded vaccination status), Travel_Times (geospatial travel time to facility), and Nightlights_Composite (development/infrastructure proxy). Panel D (bottom-right): SHAP bar chart for T2 — top-3 predictors are com_zses (community zonal SES composite), h22 (recent febrile illness), and v137 (number of children under 5 in household). Both models were trained with cluster-robust 5-fold cross-validation and post-hoc isotonic recalibration. Data source: `outputs/stage1/xgb_results_summary.json`; curves from `outputs/stage1/roc_pr_model_t1.pdf` and `roc_pr_model_t2.pdf`.
...
Panel A (Cost-Effectiveness Acceptability Curve, CEAC): probability that each scenario (S1–S5) is cost-effective relative to the status quo (S0) across a willingness-to-pay range of ₦0–₦100,000 per percentage-point increase in DTP3 coverage. At λ = ₦50,000, the risk-targeted scenario (S3) has the highest cost-effectiveness probability (0.567), followed by Uniform SMS (S1, 0.433); the RL-optimised scenario (S4) has probability 0.000 at this threshold and only becomes competitive at λ ≳ ₦70,000/pp. Panel B (Cost-Effectiveness Plane): 1,000 PSA bootstrap replicates plotted as incremental cost (₦ per child) versus incremental DTP3 coverage for each scenario versus S0, with the default willingness-to-pay threshold shown as a dashed line. Data source: `outputs/stage3_v2/ceac_data.csv`; microsimulation results from `outputs/stage3_v2/microsim_results.csv`. Default cost-effectiveness threshold: λ = ₦50,000 per percentage-point.
